...
public sealed partial class FileContentView : UserControl
{
    ...
    public static readonly DependencyProperty ContentViewModelProperty = DependencyProperty.Register(nameof(ContentViewModel), typeof(FileContentViewModel), typeof(FileContentView), new PropertyMetadata(null, OnViewModelChanged));
    public FileContentViewModel ContentViewModel { get => (FileContentViewModel)GetValue(ContentViewModelProperty); set => SetValue(ContentViewModelProperty, value); }

    // (Outras DependencyProperties: AnalyzeCommand, CopySelectedCommand mantidas...)
    public static readonly DependencyProperty AnalyzeCommandProperty = DependencyProperty.Register(nameof(AnalyzeCommand), typeof(ICommand), typeof(FileContentView), new PropertyMetadata(null));
    public ICommand AnalyzeCommand { get => (ICommand)GetValue(AnalyzeCommandProperty); set => SetValue(AnalyzeCommandProperty, value); }

    public static readonly DependencyProperty CopySelectedCommandProperty = DependencyProperty.Register(nameof(CopySelectedCommand), typeof(ICommand), typeof(FileContentView), new PropertyMetadata(null));
    public ICommand CopySelectedCommand { get => (ICommand)GetValue(CopySelectedCommandProperty); set => SetValue(CopySelectedCommandProperty, value); }
    ...
    private void BtnEdit_Click(object sender, RoutedEventArgs e)
    {
        ...
        // CORREÇÃO AQUI:
        CodeViewer.Visibility = Visibility.Collapsed; // <--- Adicione esta linha
        CodeEditor.Visibility = Visibility.Visible;
        ...
        _ = System.Threading.Tasks.Task.Delay(50, token).ContinueWith(async _ =>
        {
            ...
            try
            {
                ...
                DispatcherQueue.TryEnqueue(() =>
                {
                    ...
                    if (token.IsCancellationRequested || !_isEditing)
                        return;
                    try
                    {
                        if (ext == ".cs")
    ...
    private void MainScrollViewer_PointerWheelChanged(object sender, Microsoft.UI.Xaml.Input.PointerRoutedEventArgs e)
    {
        ...
        if (isCtrlDown)
        {
            ...
            if (mouseWheelDelta != 0)
            {
                ...
                float newZoom = mouseWheelDelta > 0 ? currentZoom + zoomStep : currentZoom - zoomStep;
