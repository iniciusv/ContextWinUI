...
public class PersistenceService : IPersistenceService
{
	...
	public async Task SaveProjectCacheAsync(
			...
			Dictionary<string, string> tagColors) // <--- Recebe aqui
	{
		try
		{
            ...
            var dto = new ProjectCacheDto
            {
				...
				Files = states.Select(s => new FileMetadataDto { RelativePath = Path.GetRelativePath(projectRootPath, s.FullPath), IsIgnored = s.IsIgnored, Tags = s.Tags.ToList() }).ToList()
            };
